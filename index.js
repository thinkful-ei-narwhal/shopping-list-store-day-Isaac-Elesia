...
const generateItemElement = function(item) {
	...
	if (item.edit) {
		itemTitle = `
    <form id="js-shopping-list-edit-item-form">
    <input type="text" name="edit-shopping-list" class="js-edit-shopping-list" placeholder="${item.name}"> 
...
/**
 * Toggles the edit for list item property
 */
...
const changeItemName = function(id, name) {
	...
	store.items[index].checked = false;
};

const handleItemEditSubmit = function() {
	$('.js-shopping-list').on(
		'submit',
		'#js-shopping-list-edit-item-form',
		event => {
			event.preventDefault();
			const newName = $('.js-edit-shopping-list').val();
			$('.js-edit-shopping-list').val('');
			const id = getItemIdFromElement(event.currentTarget);
